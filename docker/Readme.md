...
You can't do much with this toolkit without first installing reference fasta, SAM indexes and Ensembl VEP cache files to Galaxy. You can install these by obtaining the appropriate data manager tools from the main tool shed. If you are working with bam files, be sure to install a matched reference fasta. The safest (and most efficient) way to accomplish this is to provide this fasta, uncompressed, in the data folder you import to your container. The data manager for installing a reference fasta can be pointed to a local file and it should generate a symbolic link to this file when it is run this way. We have also implemented a tool that accompanies ensembl_vep (download_cache) that will allow you to install the VEP cache files needed to run this tool for a given species/build combination. By default, these will be retrieved from the ensembl ftp site (which can be slow, depending on your location). We have added a convenience option in this tool to download these files from a mirror in North America for hg19 and GRCh38.
